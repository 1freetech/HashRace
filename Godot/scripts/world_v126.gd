...
# Hash Race v0.126 live mining-capacity feedback.
# The existing capacity math was useful but mostly hidden in summaries. This
# update puts a compact operator readout directly in the playable world so a
# player can see whether the active Bitcoin mine can safely add miners before
# committing to another hardware/facility expansion.

const V126_CAPACITY_FEEDBACK_REVISION := 1
const V126_PANEL_SIZE := Vector2(286.0, 74.0)

func _ready() -> void:
    super._ready()
    set_meta("hashrace_v126_capacity_feedback_revision", V126_CAPACITY_FEEDBACK_REVISION)
    queue_redraw()
...
func _v126_draw_capacity_feedback() -> void:
    ...
    var panel := Rect2(Vector2(18.0, viewport_size.y - V126_PANEL_SIZE.y - 18.0), V126_PANEL_SIZE)
...
func debug_v126_ready() -> bool:
    ...
    return V126_CAPACITY_FEEDBACK_REVISION == 1 \
