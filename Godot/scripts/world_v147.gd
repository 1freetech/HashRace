...
# v0.147 visual declutter pass. Restores the established sparse-campus,
# one-road-language, label-budget, shadow, and compact-HUD contracts without
# replacing the current v0.145/v0.146 interaction behavior.
const V147_VISUAL_DECLUTTER_REVISION := 1
const V147_ROAD_TILE := Vector2(60.0, 54.0)
const V147_ROAD_SEGMENTS := 7
const V147_PROP_ROAD_GAP := 48.0
const V147_CLUSTER_GAP := 96.0

func _ready() -> void:
    super._ready()
    set_meta("hashrace_v147_visual_declutter", true)
    set_meta("hashrace_v147_single_road_language", true)
    set_meta("hashrace_v147_sparse_four_object_site", true)
    call_deferred("_v147_restore_compact_hud")
    queue_redraw()

func _v123_install_hud() -> void:
    _v147_restore_compact_hud()

func _v147_restore_compact_hud() -> void:
    ...
    if is_instance_valid(mining_ops_widget) and active_workspace != "ops":
        mining_ops_widget.visible = false
...
func _v123_draw_ground(campus: Rect2) -> void:
    ...
    _v128_draw_city_road(anchor, company_idx, V147_ROAD_SEGMENTS, V147_ROAD_TILE)
    _v147_draw_road_transition_band(Rect2(anchor, Vector2(V147_ROAD_TILE.x * V147_ROAD_SEGMENTS, V147_ROAD_TILE.y)))

func _v147_draw_road_transition_band(road: Rect2) -> void:
...
func _v115_draw_live_site(origin: Vector2) -> void:
    ...
    _v147_draw_energy_source(_v114_primary_energy_id(_player_hq_center()), origin + Vector2(225.0, -112.0), capacity_mw, "up")
    _v147_draw_transformer(origin + Vector2(205.0, 205.0), capacity_mw)
...
func _v147_draw_energy_source(asset_id: String, pos: Vector2, capacity_mw: float, orientation: String) -> void:
...
func _v147_draw_transformer(pos: Vector2, capacity_mw: float) -> void:
...
func debug_v147_ready() -> bool:
    return V147_VISUAL_DECLUTTER_REVISION == 1 \
        and V147_PROP_ROAD_GAP == 48.0 \
        and V147_CLUSTER_GAP == 96.0 \
        and bool(get_meta("hashrace_v147_single_road_language", false)) \
        and bool(get_meta("hashrace_v147_sparse_four_object_site", false)) \
