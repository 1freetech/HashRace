...
# Hash Race v0.054 procedural building renderer.
# Builds crisp 96x128 RGBA facade textures entirely in code, then lets the live
# overworld scale them with nearest-neighbor filtering. The source approach is
# adapted from the supplied create_building_image() pattern: masonry courses,
# roof silhouettes, doors, framed windows, foundations, vents and accent detail.
...
static func create_building_image(accent: Color = Color("39ff75"), style: String = "hq", seed: int = 0) -> Image:
    ...
    var palette := _palette_for(style, accent)
    var wall: Color = palette["wall"]
    var wall_dark: Color = palette["wall_dark"]
    var mortar: Color = palette["mortar"]
    var roof: Color = palette["roof"]
    var roof_high: Color = palette["roof_high"]
    var frame: Color = palette["frame"]
    var glass: Color = palette["glass"]
    var foundation: Color = palette["foundation"]
    var foundation_dark: Color = palette["foundation_dark"]
    var door: Color = palette["door"]

    _draw_ground_shadow(img)
    _draw_foundation(img, foundation, foundation_dark)
    _draw_wall(img, style, wall, wall_dark, mortar)
    _draw_roof(img, style, roof, roof_high, accent, seed)
    _draw_door(img, style, door, frame, accent)
    _draw_windows_or_intakes(img, style, glass, frame, accent, seed)
    _draw_style_details(img, style, accent, frame, seed)
...
static func _draw_ground_shadow(img: Image) -> void:
    for y in range(118, 124):
        ...
        for x in range(7 + inset, 89 - inset):
            if (x + y) % 2 == 0:
                img.set_pixel(x, y, Color("17212688"))
    ...
    for y in range(40, 110):
        for x in range(12, 84):
            if masonry:
                ...
                img.set_pixel(x, y, mortar.darkened(0.30) if panel_line else (wall if ((x + y) % 5 != 0) else wall_dark))
...
static func _draw_roof(img: Image, style: String, roof: Color, roof_high: Color, accent: Color, seed: int) -> void:
    if style in ["hq", "bank", "land"]:
        ...
        for y in range(27, 43):
            for x in range(7, 89):
                var roof_pixel: Color = roof_high if (x + y + seed) % 7 == 0 else roof
                img.set_pixel(x, y, roof_pixel)
        for x in range(5, 91):
    ...
    for win in windows:
        ...
        for y in range(wy + 2, wy + wh - 2):
            for x in range(wx + 2, wx + ww - 2):
                var glint: bool = (x + y + seed) % 11 == 0
                img.set_pixel(x, y, glass.lightened(0.20) if glint else glass)
    ...
    match style:
        ...
        "power":
            for x in range(13, 84, 8):
                img.set_pixel(x, 103, accent if ((x + seed) / 8) % 2 == 0 else frame)
                img.set_pixel(x + 1, 103, accent if ((x + seed) / 8) % 2 == 0 else frame)
        "bank":
