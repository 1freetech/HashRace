...
# Hash Race v0.054 building-render pass.
# Replaces flat facility blocks with cached procedural RGBA facades generated
# from code. Textures stay crisp through nearest-neighbor filtering, preserve
# the v0.052 wider spacing, and keep building labels proximity-gated.
...
func _draw_procedural_building(pos: Vector2, footprint: Vector2, style: String, accent: Color, seed: int) -> void:
    ...
    var rect := Rect2(
        VisualStack.snap_to_pixel(pos + Vector2(-draw_width * 0.5, -draw_height * 0.58)),
