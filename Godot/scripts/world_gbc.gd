...
func _ready() -> void:
	...
	_open_message(
		...
		"Hash Race now uses a tile-built company world. Roads, lots, water and grass are aligned to a Game Boy-style grid while the business simulation remains modern. Walk with WASD/arrows, click open ground to pathfind, E talks, and T travels between mining towns."
	)
	queue_redraw()
...
func _draw() -> void:
	...
	draw_string(ThemeDB.fallback_font, Vector2(1110.0, 306.0), "GRID TOWNS • ASIC MARKETS • POWER • CAPITAL", HORIZONTAL_ALIGNMENT_LEFT, -1, 11, Color("77b9c4"))
...
func _draw_art_tile(cell: Vector2i, tile_id: int) -> void:
	var p: Vector2 = Vector2(float(cell.x) * ART_TILE_SIZE, float(cell.y) * ART_TILE_SIZE)
	...
	# Four-shade pixel texture: deterministic and intentionally chunky.
	...
	if tile_id == TILE_ROAD:
		...
		draw_line(p + Vector2(8.0, 15.0), p + Vector2(28.0, 15.0), c3, 3.0)
		draw_line(p + Vector2(20.0, 34.0), p + Vector2(43.0, 34.0), c2, 3.0)
	elif tile_id == TILE_LOT:
		draw_rect(Rect2(p + Vector2(4.0, 4.0), Vector2(40.0, 40.0)), c0, false, 2.0)
	elif tile_id == TILE_PLAZA:
		draw_rect(Rect2(p + Vector2(10.0, 10.0), Vector2(28.0, 28.0)), c3, false, 2.0)
...
func _draw_road_connections(cell: Vector2i, p: Vector2, marking: Color) -> void:
	...
	if left_road or right_road:
		draw_line(center + Vector2(-22.0, 0.0), center + Vector2(22.0, 0.0), marking, 2.0)
	if up_road or down_road:
		draw_line(center + Vector2(0.0, -22.0), center + Vector2(0.0, 22.0), marking, 2.0)
...
func _draw_pixel_town_labels() -> void:
	for raw_zone in town_zones:
		...
		draw_rect(sign_rect, GBC_INK, true)
		draw_rect(sign_rect, accent, false, 3.0)
...
func _draw_world_props_pixel() -> void:
	# Solar field: repeated pixel modules.
	for row in range(2):
		for col in range(5):
			var p: Vector2 = Vector2(2130.0 + float(col) * 48.0, 245.0 + float(row) * 38.0)
			draw_rect(Rect2(p, Vector2(38.0, 26.0)), Color("0c2030"), true)
			draw_rect(Rect2(p + Vector2(4.0, 4.0), Vector2(30.0, 18.0)), Color("245985"), true)
			draw_line(p + Vector2(19.0, 4.0), p + Vector2(19.0, 22.0), Color("61a8cf"), 2.0)
	# Substation pylons become chunky readable silhouettes.
...
func _draw_tech_rep(pos: Vector2, accent: Color, scanner: String, is_player: bool) -> void:
	# Original 4-shade pixel-techwear sprite. Each block is deliberately aligned
	# to a small pixel grid so the character reads like a handheld-era RPG sprite.
	var px: float = 4.0
	...
	draw_ellipse_shadow(pos + Vector2(0.0, 31.0), 20.0, 7.0)

	# Boots / legs.
	_draw_px(pos, -3, 4, 2, 5, px, cloth0)
	_draw_px(pos, 1, 4, 2, 5, px, cloth0)
	_draw_px(pos, -4, 8, 3, 2, px, cloth2)
	_draw_px(pos, 1, 8, 3, 2, px, cloth2)
	# Jacket / shoulders.
	_draw_px(pos, -5, -2, 10, 7, px, cloth1)
	_draw_px(pos, -6, -1, 2, 5, px, cloth2)
	_draw_px(pos, 4, -1, 2, 5, px, cloth2)
	_draw_px(pos, -3, -1, 6, 5, px, cloth2)
	_draw_px(pos, 0, -1, 1, 5, px, cloth3)
	_draw_px(pos, -2, 2, 4, 1, px, cloth0)
	# Neck/head/hair.
	_draw_px(pos, -1, -4, 2, 2, px, skin1)
	_draw_px(pos, -3, -8, 6, 5, px, skin2)
	_draw_px(pos, -3, -8, 6, 1, px, cloth0)
	_draw_px(pos, -3, -7, 1, 2, px, skin0)
	_draw_px(pos, 2, -7, 1, 2, px, skin0)
	# Scanner visor on requested eye.
	var lens_x: int = -3 if scanner == "left" else 1
	_draw_px(pos, lens_x, -6, 2, 1, px, cloth3)
	_draw_px(pos, -4 if scanner == "left" else 3, -7, 1, 3, px, cloth2)
	# Tiny face highlight and belt badge.
	_draw_px(pos, -1, -5, 1, 1, px, Color("f0c09a"))
	_draw_px(pos, 2, -5, 1, 1, px, Color("f0c09a"))
	_draw_px(pos, -1, 3, 2, 1, px, cloth3)
	if is_player:
		draw_rect(Rect2(pos + Vector2(-28.0, -39.0), Vector2(56.0, 82.0)), Color(accent.r, accent.g, accent.b, 0.65), false, 2.0)
...
func _draw_power_building(entity: Dictionary, idx: int) -> void:
	...
	for x in [-48.0, 0.0, 48.0]:
		...
		draw_rect(Rect2(pos + Vector2(x - 6.0, -10.0), Vector2(12.0, 12.0)), accent, false, 3.0)
	draw_string(ThemeDB.fallback_font, pos + Vector2(-110.0, 78.0), String(entity["name"]), HORIZONTAL_ALIGNMENT_CENTER, 220.0, 11, WHITE)
...
func _draw_land_building(entity: Dictionary, idx: int) -> void:
	...
	for gx in range(3):
		for gy in range(2):
			draw_rect(Rect2(pos + Vector2(-46.0 + float(gx) * 34.0, -18.0 + float(gy) * 25.0), Vector2(24.0, 15.0)), accent.darkened(0.55), false, 2.0)
...
func _draw_pixel_facility(pos: Vector2, size_value: Vector2, accent: Color, floors: int, badge: String) -> void:
	...
	draw_rect(Rect2(pos + Vector2(-size_value.x * 0.47, size_value.y * 0.42), Vector2(size_value.x * 0.94, 16.0)), Color("00000066"), true)
	draw_rect(body, Color("0b1720"), true)
	draw_rect(Rect2(left, top, size_value.x, 12.0), accent, true)
	draw_rect(body, Color("020609"), false, 4.0)
	...
	draw_rect(Rect2(pos + Vector2(-14.0, size_value.y * 0.12), Vector2(28.0, size_value.y * 0.27)), Color("020609"), true)
	draw_rect(Rect2(pos + Vector2(-11.0, size_value.y * 0.15), Vector2(7.0, size_value.y * 0.20)), accent.darkened(0.25), true)
	draw_rect(Rect2(pos + Vector2(size_value.x * 0.28, -size_value.y * 0.76), Vector2(6.0, 35.0)), accent.darkened(0.15), true)
	draw_rect(Rect2(pos + Vector2(size_value.x * 0.23, -size_value.y * 0.80), Vector2(26.0, 7.0)), accent, true)
