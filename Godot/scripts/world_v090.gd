...
# Hash Race v0.090 critic-quality gameplay/readability pass.
# Fixes interaction geometry and input conflicts, then makes the field layer
# quieter so decisions, movement, and nearby targets read before decoration.

const V090_QUALITY_REVISION: int = 1
const V090_BUILDING_LABEL_DISTANCE: float = 210.0
const V090_TOWN_LABEL_DISTANCE: float = 300.0
...
func _ready() -> void:
    ...
    _refresh_v090_turn_hint()
    set_meta("hashrace_v090_quality_revision", V090_QUALITY_REVISION)
    queue_redraw()
...
func _process(delta: float) -> void:
    ...
    # A manual movement input clears the route in world_grid.gd. If that
    # happens before arrival, cancel the queued interaction as well.
    if nav_path.is_empty() and not has_click_target:
        pending_interaction_idx = -1

func _unhandled_input(event: InputEvent) -> void:
    # Space is interaction only. Older v0.085 code also mapped Space to end
    # turn; intercepting it here prevents accidental economic settlement.
    ...
    # Clicking an entity now means "go there and interact", not "open a remote
    # building from across the map".
...
func _queue_or_open_interaction(idx: int) -> void:
    ...
    var target := _entity_interaction_point(idx)
...
func _show_building_name(pos: Vector2, idx: int) -> bool:
    ...
    if idx != _nearest_building_idx():
        return false
    return rep_pos.distance_to(_entity_interaction_point(idx)) <= V090_BUILDING_LABEL_DISTANCE
...
func _draw_pixel_town_labels() -> void:
    ...
    for raw_zone in town_zones:
        ...
        if distance < best_distance:
            best_distance = distance
            best_zone = zone
    if best_zone.is_empty() or best_distance > V090_TOWN_LABEL_DISTANCE:
        return
...
func _draw_clean_navigation_path() -> void:
    ...
    points.append(VisualStack.snap_to_pixel(rep_pos))
    for i in range(nav_path_index, nav_path.size()):
        if i == nav_path_index or i == nav_path.size() - 1 or (i - nav_path_index) % 3 == 0:
            points.append(VisualStack.snap_to_pixel(nav_path[i]))

    if points.size() >= 2:
        draw_polyline(points, Color("4df0ff66"), 2.0, false)

    var destination := VisualStack.snap_to_pixel(nav_path[nav_path.size() - 1])
    draw_circle(destination, 9.0, Color("64ff8ccc"), false, 2.0)
    draw_circle(destination, 3.0, Color("f4f7f5"), true)

func _refresh_ui() -> void:
    super._refresh_ui()
    _refresh_v090_turn_hint()

func _refresh_v090_turn_hint() -> void:
    if is_instance_valid(quarter_button):
        quarter_button.tooltip_text = "End the current %s turn. Q previews the result; use the on-screen confirm button to settle. Space only interacts." % turn_length_name()

func debug_v090_ready() -> bool:
    return (
        V090_QUALITY_REVISION == 1
        and has_method("_entity_interaction_point")
        and has_method("_queue_or_open_interaction")
        and has_method("_draw_clean_navigation_path")
