...
func turn_length_days() -> float:
    return custom_days if turn_length_name() == "CUSTOM" else float(TURN_LENGTHS[turn_length_idx]["days"])
...
func _install_turn_scale_control() -> void:
    ...
    custom_days_spin.position = Vector2(1210,134)
    custom_days_spin.size = Vector2(200,38)
    custom_days_spin.min_value = 1
    custom_days_spin.max_value = 3650
    custom_days_spin.step = 1
...
func _scaled_financial_preview(days:float)->Dictionary:
    ...
    var power:=_machine_load_kw()*24*days*_effective_power_cost()*_uptime()
...
func _end_quarter()->void:
    if campaign_complete:
        return
    var days:=minf(turn_length_days(),maxf(0,float(campaign_years)*DAYS_PER_YEAR-elapsed_campaign_days))
    if days<=0:
        return
    ...
    quarter_button.text="END %s TURN"%turn_length_name()
    var mined:=_btc_per_day()*days
    var sats:=mined*SATS_PER_BTC
    var held:=sats*float(player["treasury_hold"])
    var p:=_scaled_financial_preview(days)
    player["sats"]=float(player["sats"])+held
    player["cash"]=float(player["cash"])+float(p["profit"])
    player["last_profit"]=float(p["profit"])
    ...
    turn+=1
    _recalculate_campaign_turns()
    _open_message("%s SETTLEMENT"%turn_length_name(),"%s TURN %d closed: %.2f days • mined %d sats • cash result $%d. %s"%[turn_length_name(),settled,days,int(sats),int(p["profit"]),note])
    _refresh_ui()
    queue_redraw()
...
func _cancel_live_quarter_confirmation()->void:
    if live_quarter_confirmation_pending:
        live_quarter_confirmation_pending=false
        quarter_button.text="END %s TURN"%turn_length_name()
        _feedback("Turn settlement cancelled.")

func _elapsed_probability(quarter_probability:float,days:float)->float:
    var p:=clampf(quarter_probability,0,1)
    return 1.0-pow(1.0-p,maxf(0,days)/DAYS_PER_QUARTER)
...
func _simulate_rivals_scaled(days:float)->void:
    ...
    for i in range(rivals.size()):
        ...
        if randf()<_elapsed_probability(0.55,days):
            rival["machines"]=int(rival["machines"])+max(1,int(round(float(randi_range(5,25))*maxf(0.1,scale))))
        if randf()<_elapsed_probability(0.25,days):
            rival["mw"]=float(rival["mw"])+0.25*maxf(1,scale)
        if randf()<_elapsed_probability(0.16,days):
            rival["acres"]=float(rival["acres"])+5.0*maxf(1,scale)
        rivals[i]=rival

func _advance_market_scaled(days:float,halving_happened:bool)->String:
    var scale:=sqrt(days/DAYS_PER_QUARTER)
    federal_rate=clampf(federal_rate+randf_range(-0.0035,0.0035)*scale,0.005,0.085)
    btc_price=maxf(8000,btc_price*maxf(0.72,1+randf_range(-0.12,0.18)*scale))
    land_price_per_acre=maxf(1800,land_price_per_acre*maxf(0.80,1+randf_range(-0.025,0.040)*scale))
    energy_market_index=clampf(energy_market_index*(1+randf_range(-0.06,0.07)*scale),0.72,1.45)
    network_hashrate_th*=maxf(0.90,1+randf_range(-0.015,0.075)*scale)
    average_fees_btc=clampf(average_fees_btc*maxf(0.50,1+randf_range(-0.28,0.38)*scale),0.04,0.85)
    return "Fed %.2f%% • BTC $%d • land $%d/acre."%[federal_rate*100,int(btc_price),int(land_price_per_acre)]

func _refresh_ui()->void:
    super._refresh_ui()
    if not is_instance_valid(top_stats) or player.is_empty():
        return
    var year:=mini(campaign_years,int(elapsed_campaign_days/DAYS_PER_YEAR)+1)
    var day:=int(fmod(elapsed_campaign_days,DAYS_PER_YEAR))+1
    var q:=clampi(int((day-1)/(DAYS_PER_YEAR/4))+1,1,4)
    top_stats.text="%s | Y%d Q%d DAY %d | TURN %d | %s | Cash $%d | SATS %d"%[String(player["name"]),year,q,day,turn,turn_length_name(),int(player["cash"]),int(player["sats"])]
