...
# Hash Race v0.145 visual declutter pass. Restores v0.091/v0.094/v0.103/v0.114/v0.128 visual contracts without touching simulation state.
const V145_VISUAL_DECLUTTER_REVISION := 1
const V145_ROAD_TILE := Vector2(60.0, 54.0)
const V145_ROAD_SEGMENTS := 7
const V145_PROP_ROAD_GAP := 48.0
const V145_CLUSTER_GAP := 96.0

func _ready() -> void:
    super._ready()
    set_meta("hashrace_v145_visual_declutter", true)
    set_meta("hashrace_v145_single_road_language", true)
    set_meta("hashrace_v145_sparse_four_object_site", true)
    call_deferred("_v145_restore_compact_hud")
    queue_redraw()

func _v123_install_hud() -> void:
    _v145_restore_compact_hud()

func _v145_restore_compact_hud() -> void:
...
func _v123_draw_ground(campus: Rect2) -> void:
    ...
    _v128_draw_city_road(anchor, company_idx, V145_ROAD_SEGMENTS, V145_ROAD_TILE)
    _v145_draw_road_transition_band(Rect2(anchor, Vector2(V145_ROAD_TILE.x * V145_ROAD_SEGMENTS, V145_ROAD_TILE.y)))

func _v145_draw_road_transition_band(road: Rect2) -> void:
...
func _v115_draw_live_site(origin: Vector2) -> void:
    ...
    _v145_draw_energy_source(_v114_primary_energy_id(_player_hq_center()), origin + Vector2(225.0, -112.0), capacity_mw, "up")
    _v145_draw_transformer(origin + Vector2(205.0, 205.0), capacity_mw)
...
func _v145_draw_energy_source(asset_id: String, pos: Vector2, capacity_mw: float, orientation: String) -> void:
...
func _v145_draw_transformer(pos: Vector2, capacity_mw: float) -> void:
...
func debug_v145_ready() -> bool:
    return V145_VISUAL_DECLUTTER_REVISION == 1 and V145_PROP_ROAD_GAP == 48.0 and V145_CLUSTER_GAP == 96.0 and bool(get_meta("hashrace_v145_single_road_language", false)) and bool(get_meta("hashrace_v145_sparse_four_object_site", false)) and debug_v144_ready()
