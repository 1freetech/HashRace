...
func _run() -> void:
    ...
    var required_methods: Array = [
        ...
        "debug_negotiation_ready", "debug_negotiation_snapshot", "debug_v072_ready", "debug_v073_ready", "debug_v080_ready", "debug_v082_ready", "debug_v090_ready", "debug_computer_offer_ready", "debug_computer_offer_snapshot", "debug_pixel_integration_ready", "debug_character_pose_library", "debug_character_body_variant_count", "play_character_action", "start_negotiation",
    ...
    if not bool(scene.call("debug_pixel_integration_ready")):
        ...
        return
    if not bool(scene.call("debug_v082_ready")):
        _fail("v0.082 8x8 microtile terrain/building integration is not ready")
        return
    if not bool(scene.call("debug_v090_ready")):
        _fail("v0.090 interaction/readability quality layer did not initialize")
        return
    ...
    print("HASH RACE OVERWORLD PASS: v0.082 verified 8x8 microtile terrain/building composition on top of the v0.080 hard-grid camera/depth contract, Resource-backed infrastructure, physical rack placement, fixed-tick simulation, consolidated live metrics, and negotiation systems.")
    quit(0)
