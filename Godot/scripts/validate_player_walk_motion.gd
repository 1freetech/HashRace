...
# At 144 px/s a seven-pose walk at 8 FPS advances 18 actual pixels per
# displayed pose; one full walk cycle therefore spans exactly 126 pixels.
...
func _validate() -> void:
    ...
    _require(Sheet.WALK_FRAME_COUNT == 7 and is_equal_approx(Sheet.WALK_FPS, 8.0), "seven walk frames at 8 FPS")
    _require(Sheet.EFFECTIVE_FRAME_COUNT == 32 and Sheet.EFFECTIVE_SOURCE_INDICES == [0, 1, 2, 3, 4, 5, 6, 7], "approved source must preserve every authored walk pose plus idle")
    _require(is_equal_approx(Movement.WALK_CYCLE_DISTANCE, 126.0), "walking cycle must span 126 traveled pixels")
    ...
    for facing in ["down", "left", "right", "up"]:
        ...
        _require(frames.get_frame_count(animation) == 7, facing + " must have seven walking poses")
        ...
        _require(visited.size() == 7, facing + ": every walk pose must be reached")
    ...
    if not failures.is_empty():
        quit(1)
        return
    print("HASH RACE WALK MOTION PASS: live 144 px/s; 7 walk frames per direction; 8 FPS; 126 px cycle; real-distance phases; 4-direction idle reset")
    quit(0)
