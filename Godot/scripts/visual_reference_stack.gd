...
# Hash Race v0.030 visual-reference utility.
# Each helper independently adapts one small, general 2D rendering technique
# observed in the listed upstream projects. No third-party runtime is bundled.
# Spine and Luxor are treated as concept references only because GitHub does not
# report a standard SPDX license for those repositories.
