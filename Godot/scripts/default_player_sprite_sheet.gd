...
# The approved sheet contains one idle plus seven authored movement poses per
# direction. Preserve their visual order. Skipping to odd-numbered source
# frames drops the in-between opposite-leg poses and makes lateral walking slide.
const FRAME_REGIONS := {
...
static func _retint(source: Color, target: Color, reference_value: float) -> Color:
    ...
    return Color.from_hsv(target.h, target.s, clampf(target.v * shade, 0.0, 1.0), source.a)
...
static func build_frames() -> SpriteFrames:
    ...
    for facing in ["down", "left", "right", "up"]:
        ...
        _add_animation(frames, StringName("walk_" + facing), texture, regions.slice(1, 8), WALK_FPS, true)
    return frames
...
static func debug_ready() -> bool:
    var texture := load_texture()
    return texture != null and Vector2i(texture.get_size()) == SHEET_SIZE and FRAME_REGIONS.size() == 4 and FRAME_REGIONS["down"].size() == 8 and EFFECTIVE_SOURCE_INDICES.size() * FRAME_REGIONS.size() == EFFECTIVE_FRAME_COUNT
