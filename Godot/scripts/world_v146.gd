...
# Hash Race v0.146: forward-port the v0.141 visual declutter contracts onto
# the current v0.145 live-world chain. Simulation state and v0.145 interaction UX stay intact.
const V146_VISUAL_DECLUTTER_REVISION := 1
const V146_ROAD_TILE := Vector2(60.0, 54.0)
const V146_ROAD_SEGMENTS := 7
const V146_PROP_ROAD_GAP := 48.0
const V146_CLUSTER_GAP := 96.0

func _ready() -> void:
    super._ready()
    set_meta("hashrace_v146_visual_declutter", true)
    set_meta("hashrace_v146_single_road_language", true)
    set_meta("hashrace_v146_sparse_four_object_site", true)
    call_deferred("_v146_restore_compact_hud")
    queue_redraw()

func _v123_install_hud() -> void:
    _v146_restore_compact_hud()

func _v146_restore_compact_hud() -> void:
...
func _v123_draw_ground(campus: Rect2) -> void:
    ...
    _v128_draw_city_road(anchor, company_idx, V146_ROAD_SEGMENTS, V146_ROAD_TILE)
    _v146_draw_road_transition_band(Rect2(anchor, Vector2(V146_ROAD_TILE.x * V146_ROAD_SEGMENTS, V146_ROAD_TILE.y)))

func _v146_draw_road_transition_band(road: Rect2) -> void:
...
func _v115_draw_live_site(origin: Vector2) -> void:
    ...
    _v146_draw_energy_source(_v114_primary_energy_id(_player_hq_center()), origin + Vector2(225.0, -112.0), capacity_mw, "up")
    _v146_draw_transformer(origin + Vector2(205.0, 205.0), capacity_mw)
...
func _v146_draw_energy_source(asset_id: String, pos: Vector2, capacity_mw: float, orientation: String) -> void:
...
func _v146_draw_transformer(pos: Vector2, capacity_mw: float) -> void:
...
func debug_v146_ready() -> bool:
    return V146_VISUAL_DECLUTTER_REVISION == 1 \
        and V146_PROP_ROAD_GAP == 48.0 \
        and V146_CLUSTER_GAP == 96.0 \
        and bool(get_meta("hashrace_v146_single_road_language", false)) \
        and bool(get_meta("hashrace_v146_sparse_four_object_site", false)) \
