...
static func snap_to_grid(world_position: Vector2, grid_size: Vector2i = Vector2i(32, 32)) -> Vector2:
    var gx := max(1, grid_size.x)
    var gy := max(1, grid_size.y)
    return Vector2(
        round(world_position.x / float(gx)) * float(gx),
        round(world_position.y / float(gy)) * float(gy)
    )

static func within_work_distance(origin: Vector2, target: Vector2, maximum_distance := 400.0) -> bool:
    return origin.distance_to(target) <= maximum_distance

static func ghost_color(valid: bool) -> Color:
    return VALID if valid else INVALID

static func cell_is_free(candidate: Rect2, occupied: Array[Rect2], clearance := 6.0) -> bool:
    var padded := candidate.grow(clearance)
