...
for marker in ["class_name HashRaceItemResource","@export var id:","@export var display_name:","@export var slot_type:","@export var base_hashrate_ph:","@export var power_draw_mw:","@export var heat_generated_mw:","func to_legacy_dict()"]:
...
for required_id in ["asic_gen1","asic_s19","asic_s21","asic_hydro","transformer_1mw","gas_turbine","solar_array","nuclear_smr","immersion_tank","hydro_loop","container","fiber","repair_lab","backup_gen","semi_deal"]:
...
assert "const CATALOG" not in inventory
for marker in ["ItemLibrary.load_catalog","catalog_resources","catalog_item_at","item_resource","debug_resource_catalog_ready"]:
    assert marker in inventory, marker
for marker in ["class_name HashRaceRackSlot","installed_hardware","can_install","install_item","remove_item"]:
    assert marker in rack_slot, marker
for marker in ["class_name HashRaceRackContainer","rebuild_slots","first_free_slot","aggregate_live_stats"]:
    assert marker in rack_container, marker
for marker in ["class_name HashRacePhysicalPlacementGrid","snap(","can_place","place(","serialize_layout","clear_all"]:
    assert marker in placement, marker
for marker in ["class_name HashRaceSimulationManager","signal tick_processed","Timer.new()","timer.wait_time = tick_rate","func process_tick()",'"power_ratio"','"thermal_factor"','"effective_hashrate"']:
    assert marker in simulation, marker
for marker in ["SimulationManager","PhysicalPlacementGrid","RackContainer","_sync_physical_racks","_on_simulation_tick","debug_modular_architecture_ready","debug_simulation_snapshot"]:
