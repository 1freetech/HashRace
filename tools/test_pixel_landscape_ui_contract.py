...
assert 'extends "res://scripts/world_v059.gd"' in world_v065
assert 'extends "res://scripts/world_v055.gd"' in world_v059
assert 'extends "res://scripts/world_v053.gd"' in world_v055
assert 'extends "res://scripts/world_ui_compact.gd"' in texture_spacing
for marker in ["MICRO_TILE_SIZE: int = 16","Image.create(MICRO_TILE_SIZE, MICRO_TILE_SIZE","img.set_pixel","ImageTexture.create_from_image","CanvasItem.TEXTURE_FILTER_NEAREST","_stamp_campus_walkways","_draw_walkway_edges","_draw_road_curbs","_draw_water_bank"]:
    assert marker in landscape, marker
for marker in ['"MENU  [M]"','"CONTROL CENTER"','"COMPANY"','"BTC TREASURY"','"LIFE + SITE"','"LEAGUE STANDINGS"','"MARKET INFO"','"NEXT MINING TOWN  [T]"','scanner_overlay_enabled = false','layer.visible = false','_clear_detail_panels']:
