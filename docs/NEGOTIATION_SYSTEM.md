...
The first live deal type is a small flexible-power access contract. A successful agreement transfers cash from the player to the rival and adds the negotiated MW capacity to the player.\n\nHash Race v0.086 also lets external computer companies initiate supplier offers during normal play. These offers are not tied to turn settlement: a randomized real-time timer can open a negotiation while the player is operating in the world. Computer-company deals can deliver current-generation machines and a bounded fleet-efficiency bonus when successfully negotiated.
...
- `Godot/scripts/world_v072.gd` — converts live rival/company state into rival negotiation context and applies successful capacity deals.\n- `Godot/scripts/world_v086.gd` — schedules proactive computer-company supplier offers and applies computer-hardware rewards.
